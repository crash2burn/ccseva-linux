...
export const Analytics: React.FC<AnalyticsProps> = ({ stats }) => {
  ...
  const maxValue = useMemo(() => {
    ...
    const values = chartData.map(d => selectedMetric === 'tokens' ? d.totalTokens : d.totalCost);
    ...
    <div className="space-y-4">
      ...
      <div className="glass-card p-5">
        ...
        <div className="flex flex-wrap gap-3 mb-5">
          ...
          <div className="flex bg-neutral-800/50 rounded-xl p-1 backdrop-blur-sm border border-white/10">
            {([
              { type: 'area', label: 'Area', icon: 'M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9' },
          ...
          <div className="flex items-center gap-2">
            <div className="glass px-3 py-1 rounded-lg">
              <span className="text-xs text-neutral-300">
                Max: {selectedMetric === 'tokens' ? formatNumber(maxValue) : formatCurrency(maxValue)}
              </span>
            </div>
          </div>
        </div>
            ...
            >
              <defs>
                <linearGradient id="chartGradient" x1="0%" y1="0%" x2="0%" y2="100%">
                  <stop offset="0%" stopColor={selectedMetric === 'tokens' ? '#3B82F6' : '#10B981'} stopOpacity={0.3} />
                  <stop offset="100%" stopColor={selectedMetric === 'tokens' ? '#3B82F6' : '#10B981'} stopOpacity={0.05} />
                </linearGradient>
              </defs>
              ...
              {[1, 0.75, 0.5, 0.25, 0].map((ratio) => {
                ...
                const displayValue = selectedMetric === 'tokens' ? formatNumber(value) : formatCurrency(value);
                  ...
                  {chartType === 'bar' && (
                    <>
                      {chartData.map((d, i) => {
                        ...
                        const x = padding + (i * plotWidth / chartData.length) + (plotWidth / chartData.length - barWidth) / 2;
                        const value = selectedMetric === 'tokens' ? d.totalTokens : d.totalCost;
                        const normalizedValue = maxValue > 0 ? value / maxValue : 0;
              ...
              {chartData.map((d, i) => {
                const x = chartType === 'bar' 
                  ? padding + (i * plotWidth / chartData.length) + (plotWidth / chartData.length) / 2
            ...
            <div className="flex items-center gap-8">
              ...
              <div className="relative w-40 h-40 flex-shrink-0">
                <svg width="160" height="160" className="transform -rotate-90">
                  ...
                  {modelBreakdownData.map((model, index) => {
                    ...
                    const strokeDashoffset = circumference - (model.percentage / 100) * circumference;
                    const rotation = modelBreakdownData.slice(0, index).reduce((sum, prev) => sum + prev.percentage, 0) * 3.6;
              ...
              <div className="flex-1 space-y-3">
                {modelBreakdownData.map((model) => (
                  <div key={model.name} className="flex items-center justify-between p-4 glass rounded-xl">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <div className="text-sm text-neutral-400">{model.percentage.toFixed(1)}% usage</div>
                      </div>
                    </div>
